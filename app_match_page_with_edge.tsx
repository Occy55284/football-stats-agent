...
function formatPct(value?: number | null) {
  return `${firstNumber(value).toFixed(1)}%`;
}

function readSnapshotPct(obj: any, key: string) {
  const value = obj?.[key];
  if (typeof value === "number") return value;
  return Number(value || 0);
}

function buildMarketEdgeRows(prediction: PredictionRow | null, homeTeam?: TeamRow | null, awayTeam?: TeamRow | null) {
  const snapshot = prediction?.input_snapshot || null;
  const market = snapshot?.market_probabilities || null;

  if (!prediction || !market) return [] as MarketEdgeRow[];

  const rows: MarketEdgeRow[] = [
    {
      label: `${homeTeam?.name || "Home"} win`,
      model: firstNumber(prediction.home_win_pct),
      market: readSnapshotPct(market, "home"),
      edge: firstNumber(prediction.home_win_pct) - readSnapshotPct(market, "home"),
      tone: "blue",
    },
    {
      label: "Draw",
      model: firstNumber(prediction.draw_pct),
      market: readSnapshotPct(market, "draw"),
      edge: firstNumber(prediction.draw_pct) - readSnapshotPct(market, "draw"),
      tone: "amber",
    },
    {
      label: `${awayTeam?.name || "Away"} win`,
      model: firstNumber(prediction.away_win_pct),
      market: readSnapshotPct(market, "away"),
      edge: firstNumber(prediction.away_win_pct) - readSnapshotPct(market, "away"),
      tone: "teal",
    },
  ];
...
export default async function MatchDetailsPage({ params, searchParams }: PageProps) {
  ...
  const selectedCompetition =
    (resolvedSearchParams.competition || fixture.league_code || "PL").toUpperCase();
  ...
  const marketEdgeRows = buildMarketEdgeRows(prediction, homeTeam, awayTeam);
  const bestPositiveEdge =
    marketEdgeRows
      .filter((row) => row.edge > 0)
      .sort((a, b) => b.edge - a.edge)[0] || null;
  const marketWeights = prediction?.input_snapshot?.blended_weights || null;
  const marketOdds = prediction?.input_snapshot?.odds_market || null;
  const marketAvailable = marketEdgeRows.length > 0;
    ...
    >
      <div style={{ maxWidth: "1260px", margin: "0 auto" }}>
        <div style={{ marginBottom: "18px" }}>
          <Link
            href={`/?competition=${selectedCompetition}`}
            style={{
              display: "inline-flex",
              alignItems: "center",
              textDecoration: "none",
                ...
                <>
                  ...
                  {marketAvailable ? (
                    ...
                    >
                      ...
                      >
                        <div>
                          ...
                          <div style={{ fontSize: "13px", color: "#6b7280", lineHeight: 1.5 }}>
                            Positive edge means your blended model is higher than the bookmaker market consensus.
                          </div>
                        </div>
                        ...
                        {bestPositiveEdge ? (
                          ...
                          >
                            Best edge: {bestPositiveEdge.label} {bestPositiveEdge.edge >= 0 ? "+" : ""}{bestPositiveEdge.edge.toFixed(1)}%
                          </div>
                        ) : (
                          <div
                            style={{
                      ...
                      >
                        {marketEdgeRows.map((row) => {
                          const styles = insightToneStyles(row.tone);
                          return (
                            <div
                              key={row.label}
                              style={{
                            ...
                            >
                              ...
                              >
                                <div style={{ display: "flex", justifyContent: "space-between", gap: "10px" }}>
                                  <span>Model</span>
                                  <strong>{formatPct(row.model)}</strong>
                                </div>
                                <div style={{ display: "flex", justifyContent: "space-between", gap: "10px" }}>
                                  <span>Market</span>
                                  <strong>{formatPct(row.market)}</strong>
                                </div>
                                <div style={{ display: "flex", justifyContent: "space-between", gap: "10px" }}>
                                  <span>Edge</span>
                                  <strong style={{ color: row.edge >= 0 ? "#166534" : "#991b1b" }}>
                      ...
                      >
                        <strong style={{ color: "#0f172a" }}>Blend used:</strong>{" "}
                        {marketWeights
                          ? `${Math.round((Number(marketWeights.internal || 0) * 100))}% internal model, ${Math.round((Number(marketWeights.market || 0) * 100))}% bookmaker market, ${Math.round((Number(marketWeights.form_strength || 0) * 100))}% strength/form layer`
                          : "Internal model and bookmaker market blend"}
                        {marketOdds ? (
                          <>
                            {" "}• Market avg odds: {marketOdds.market_avg_home_odds ?? "-"} / {marketOdds.market_avg_draw_odds ?? "-"} / {marketOdds.market_avg_away_odds ?? "-"}
                          </>
                        ) : null}
                      </div>
                    </div>
